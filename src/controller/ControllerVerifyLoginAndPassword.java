...
public class ControllerVerifyLoginAndPassword {
	...
	private static final String MESSAGE_INVALID_INPUT_DATA_LOGIN_AND_PASSWORD = "Неверный логин или пароль";
	...
	// ----------------------------------------------
	public ControllerVerifyLoginAndPassword(AdministrativeInput administrativeInput, ManagingWindow managingWindow,
			ControllerManagingWindow controllerManagingWindow) {
	...
	public void verify() {
		LOG.debug(
				"запущен метод - verify(); (Метод оккрывает окошко для ввода login and password), в классе -  ControllerVerifyLoginAndPassword");
	...
	private void checkedLogin() {
		LOG.debug(
				"запущен метод - checkedLogin(); (Метод проверяет login and password введенный пользователем), в классе -  ControllerVerifyLoginAndPassword");
		...
		for (Employee employee : workingStaff) {
			if (employee instanceof Administration) {
				if (login.equals(((Administration) employee).getLogin())) {
					...
					JOptionPane.showMessageDialog(administrativeInput, MESSAGE_INVALID_INPUT_DATA_LOGIN_AND_PASSWORD);
				}*/
			}
		}
	}
	...
	private void checkCancel() {
		LOG.debug(
				"запущен метод - checkCancel(); (Метод отменяющий данные введеные пользователем), в классе -  ControllerVerifyLoginAndPassword");
